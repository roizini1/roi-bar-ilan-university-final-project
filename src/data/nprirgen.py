...
def np_generateRir(roomMeasures, sourcePosition, receiverPositions, *, reverbTime=None, betaCoeffs=None, soundVelocity=340, fs=16000, orientation=[.0, .0], isHighPassFilter=True, nDim=3, nOrder=-1, nSamples=-1, micType='omnidirectional'):
    ...
    if betaCoeffs is None:
        ...
        S = 2 * (roomMeasures[0]*roomMeasures[2] + roomMeasures[1]*roomMeasures[2] + roomMeasures[0]*roomMeasures[1])
    ...
    if reverbTime is None:
        V = np.prod(roomMeasures)
        alpha = ((1 - betaCoeffs[0]**2) + (1 - betaCoeffs[1]**2)) * roomMeasures[1] * roomMeasures[2] + ((1 - betaCoeffs[2]**2) + (1 - betaCoeffs[3]**2)) * roomMeasures[0] * roomMeasures[2] + ((1 - betaCoeffs[4]**2) + (1 - betaCoeffs[5]**2)) * roomMeasures[0] * roomMeasures[1]
    ...
    # Now, generate RIRs. 
    import gen
    h = gen.genrir(soundVelocity, fs, receiverPositions, sourcePosition, roomMeasures, betaCoeffs, orientation, isHighPassFilter, nOrder, nSamples, micType)
